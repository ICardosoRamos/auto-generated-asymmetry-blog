...
export const scheduleDailyArticleJob = (cronExpression?: string) => {
  const expression = cronExpression && cronExpression.trim().length > 0 ? cronExpression : "0 9 * * *";

  return cron.schedule(expression, async () => {
    console.log(
      `[cron] Starting daily article generation at ${new Date().toISOString()}`
    );
    try {
